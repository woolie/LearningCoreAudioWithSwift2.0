...
let MIDINotifyCallback: MIDINotifyProc =
{
    message, refCon in print("MIDI notify, messageID=\(message.memory.messageID.rawValue)")
}

let MIDIReadCallback: MIDIReadProc =
{
    pktlist, refCon, connRefCon in
    ...
    for i in 0 ..< Int(pktlist.memory.numPackets)
    {
        ...
        if midiCommand == 0x08 || midiCommand == 0x09
        {
            ...
            CheckError(MusicDeviceMIDIEvent(player.memory.instrumentUnit, UInt32(midiStatus), UInt32(note), UInt32(velocity), 0), operation: "Couldn't send MIDI event")
...
{
    CheckError(NewAUGraph(&player.graph), operation: "Couldn't open AUGraph")
    ...
    CheckError(AUGraphAddNode(player.graph, &instrumentcd, &instrumentNode), operation: "AUGraphAddNode[kAudioUnitSubType_DLSSynth] failed")
    ...
    CheckError(AUGraphNodeInfo(player.graph, instrumentNode, nil, &player.instrumentUnit), operation: "AUGraphNodeInfo failed")
    
    // Connect the output source of the speech synthesis AU to the input source of the output node
    CheckError(AUGraphConnectNodeInput(player.graph, instrumentNode, 0, outputNode, 0), operation: "AUGraphConnectNodeInput failed")
...
func setupMIDI(inout player: MIDIPlayer)
{
    ...
    CheckError(MIDIClientCreate("Core MIDI to System Sounds Demo", MIDINotifyCallback, &player, &client), operation: "Couldn't create MIDI client")
    
    var inPort = MIDIPortRef()
    CheckError(MIDIInputPortCreate(client, "Input port", MIDIReadCallback, &player, &inPort), operation: "Couldn't create MIDI input port")
    ...
    for i in 0..<sourceCount
    {
        ...
        CheckError(MIDIObjectGetStringProperty(src, kMIDIPropertyName, &endpointName), operation: "Couldn't get endpoint name")

        print(" source \(i): \(endpointName!.takeRetainedValue() as String)\n")

        CheckError(MIDIPortConnectSource(inPort, src, nil), operation: "Couldn't connect MIDI port")
    }
}
...
func main()
{
    ...
    CFRunLoopRun()  // Run until aborted witgh Control-C
}
