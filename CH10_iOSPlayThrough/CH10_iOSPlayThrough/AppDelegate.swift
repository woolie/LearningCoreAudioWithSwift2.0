...
// MARK: callbacks
let InputModulatingRenderCallback: AURenderCallback =
{
    ...
    var bus1: UInt32 = 1
    CheckError(AudioUnitRender(effectState.rioUnit, ioActionFlags, inTimeStamp, bus1, inNumberFrames, ioData), operation: "Couldn't render from RemoteIO unit")
...
class AppDelegate: UIResponder, UIApplicationDelegate
{
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool
    {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AppDelegate.interruptionListener(_:)), name: AVAudioSessionInterruptionNotification, object: nil)
        ...
        CheckError(AudioComponentInstanceNew(rioComponent, &effectState.rioUnit), operation: "Couldn't get RIO unit instance")
        
        // Configure Rio unit
        // Set up the Rio unit for playback
        var oneFlag: UInt32 = 1
        let bus0: AudioUnitElement = 0
        CheckError(AudioUnitSetProperty(effectState.rioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, bus0, &oneFlag, UInt32(sizeof(oneFlag.dynamicType))), operation: "Couldn't enable RIO output")
        
        // Enable RIO input
        let bus1: AudioUnitElement = 1
        CheckError(AudioUnitSetProperty(effectState.rioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, bus1, &oneFlag, UInt32(sizeof(oneFlag.dynamicType))), operation: "Couldn't enable RIO input")
        ...
        CheckError(AudioUnitSetProperty(effectState.rioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, bus0, &asbd, UInt32(sizeof(asbd.dynamicType))), operation: "Couldn't set the ASBD for RIO on input scope/bus 0")
        
        // Set ASBD for mic input (bus 1) on RIO's output scope
        CheckError(AudioUnitSetProperty(effectState.rioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, bus1, &asbd, UInt32(sizeof(asbd.dynamicType))), operation: "Couldn't set the ASBD for RIO on output scope/bus 1")
        ...
        CheckError(AudioUnitSetProperty(effectState.rioUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Global, bus0, &callbackStruct, UInt32(sizeof(callbackStruct.dynamicType))), operation: "Couldn't set RIO's render callback on bus 0")
        
        // Initialize and start the RIO unit
        CheckError(AudioUnitInitialize(effectState.rioUnit), operation: "Couldn't initialize the RIO unit")
        CheckError(AudioOutputUnitStart(effectState.rioUnit), operation: "Couldn't start the RIO unit")
        print("RIO started!\n")
