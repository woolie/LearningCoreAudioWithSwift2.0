...
func CreateAUGraph(inout player: AUGraphPlayer)
{
    ...
    CheckError(NewAUGraph(&player.graph), operation: "NewAUGraph failde")
    ...
    CheckError(AUGraphNodeInfo(player.graph, speechNode, nil, &player.speechAU), operation: "AUGraphNodeInfo failed")
    ...
    CheckError(AUGraphAddNode(player.graph, &reverbcd, &reverbNode), operation: "AUGraphAddNode[kAudioUnitSubType_MatrixReverb] failed")
    
    // Connect the output source of the speech synthesizer AU to the input source of the reverb node
    CheckError(AUGraphConnectNodeInput(player.graph, speechNode, 0, reverbNode, 0), operation: "AUGraphConnectNodeInput (speech to reverb) failed")
    
    // Connect the output source of the reverb AU to the input source of the output node
    CheckError(AUGraphConnectNodeInput(player.graph, reverbNode, 0, outputNode, 0), operation: "AUGraphConnectNodeInput (reverb to output) failed")
    
    // Get the reference to the AudioUnit object for the reverb graph node
    var reverbUnit: AudioUnit = nil
    
    CheckError(AUGraphNodeInfo(player.graph, reverbNode, nil, &reverbUnit), operation: "AUGraphNodeInfo failed")
    ...
    CheckError(AudioUnitSetProperty(reverbUnit, kAudioUnitProperty_ReverbRoomType, kAudioUnitScope_Global, 0, &roomType, UInt32(sizeof(roomType.dynamicType))), operation: "AudioUnitSetProperty[kAudioUnitProperty_ReverbRoomType] failed")
    
    #else
    // Connect the output source of the speech synthesis AU to the input source of the output node
    CheckError(AUGraphConnectNodeInput(player.graph, speechNode, 0, outputNode, 0), operation: "AUGraphConnectNodeInput failed")
...
func PrepareSpeechAU(inout player: AUGraphPlayer)
{
    ...
    CheckError(AudioUnitGetProperty(player.speechAU, kAudioUnitProperty_SpeechChannel, kAudioUnitScope_Global, 0, &chan, &propSize), operation: "AudioUnitGetProperty[kAudioUnitProperty_SpeechChannel] failed")
