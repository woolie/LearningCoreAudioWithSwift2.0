...
let AQOutputCallback: AudioQueueOutputCallback =
{
    inUserData, inAQ, inBuffer in

    var userData = UnsafeMutablePointer<UserData>(inUserData)
    
    CheckError(userData.memory.fillBuffer(inBuffer), operation: "Can't refill buffer")
    CheckError(AudioQueueEnqueueBuffer(inAQ, inBuffer, 0, nil), operation: "Couldn't enqueue the buffer (refill)")
}
...
class AppDelegate: UIResponder, UIApplicationDelegate
{
                ...
                if let type = AVAudioSessionInterruptionType(rawValue: intValue)
                {
                    ...
                    switch type
                    {
                        ...
                        case .Ended:
                            ...
                            CheckError(AudioQueueStart(userData.audioQueue, &timeStamp), operation: "Couldn't restart the audio queue")
                    }
                }
                else
                {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool
    {
        ...
        CheckError(AudioQueueNewOutput(&userData.streamFormat!, AQOutputCallback, &userData, nil, kCFRunLoopCommonModes, 0, &userData.audioQueue), operation: "Couldn't create the output AudioQueue")
        ...
        {
            CheckError(AudioQueueAllocateBuffer(userData.audioQueue, userData.bufferSize, &buffers[i]), operation: "Couldn't allocate the Audio Queue buffer")
            CheckError(userData.fillBuffer(buffers[i]), operation: "Couldn't fill buffer (priming)")
            CheckError(AudioQueueEnqueueBuffer(userData.audioQueue, buffers[i], 0, nil), operation: "Couldn't enqueue buffer (priming)")
        }

        // Start the audio queue
        CheckError(AudioQueueStart(userData.audioQueue, nil), operation: "Couldn't start the Audio Queue")
    ...
    func applicationWillEnterForeground(application: UIApplication)
    {
        ...
        CheckError(AudioQueueStart(userData.audioQueue, &timeStamp), operation: "Couldn't restart audio queue")
