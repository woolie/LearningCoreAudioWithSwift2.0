...
class ViewController: UIViewController
{
    ...
    func connectToHost()
    {
        ...
        if midiSession != nil
        {
            ...
            CheckError(MIDIClientCreate("MIDIWifi Client", nil, nil, &client), operation: "Couldn't create MIDI client")
            CheckError(MIDIOutputPortCreate(client, "MIDIWifi Output port", &outport), operation: "Couldn't create output port")
    ...
    func sendStatus(status: UInt8, data1: UInt8, data2: UInt8)
    {
        ...
        CheckError(MIDISend(outputPort, destinationEndpoint, &packetList), operation: "Couldn't send MIDI packet list")
    }
