...
func CreateAUGraph(inout player: AUGraphPlayer)
{
    ...
    // Adds a node with above description to the graph
    var fileNode = AUNode()
    CheckError(AUGraphAddNode(player.graph, &fileplayercd, &fileNode), operation: "AUGraphAddNode[kAudioUnitSubType_AudioFilePlayer] failed")
    
    // Opening the graph opens all contained audio units but does not allocate any resources yet
    CheckError(AUGraphOpen(player.graph), operation: "AUGraphOpen failed")
    
    // Get the reference to the AudioUnit object for the file player graph node
    CheckError(AUGraphNodeInfo(player.graph, fileNode, nil, &player.fileAU), operation: "AUGraphNodeInfo failed")
    
    // Connect the output source of the file player AU to the input source of the output node
    CheckError(AUGraphConnectNodeInput(player.graph, fileNode, 0, outputNode, 0), operation: "AUGraphConnectNodeInput failed")
...
func PrepareFileAU(inout player: AUGraphPlayer) -> Double
{
    ...
    CheckError(AudioUnitSetProperty(player.fileAU, kAudioUnitProperty_ScheduledFileIDs, kAudioUnitScope_Global, 0, &player.inputFile, UInt32(sizeof(player.inputFile.dynamicType))), operation: "AudioUnitSetProperty[kAudioUnitProperty_ScheduledFileIDs] failed")
    
    var nPackets = UInt64()
    var propSize = UInt32(sizeof(nPackets.dynamicType))
    CheckError(AudioFileGetProperty(player.inputFile, kAudioFilePropertyAudioDataPacketCount, &propSize, &nPackets), operation: "AudioFileGetProperty[kAudioFilePropertyAudioDataPacketCount] failed")
    ...
    CheckError(AudioUnitSetProperty(player.fileAU, kAudioUnitProperty_ScheduledFileRegion, kAudioUnitScope_Global, 0, &rgn, UInt32(sizeof(rgn.dynamicType))), operation: "AudioUnitSetProperty[kAudioUnitProperty_ScheduledFileRegion] failed")
    ...
    CheckError(AudioUnitSetProperty(player.fileAU, kAudioUnitProperty_ScheduleStartTimeStamp, kAudioUnitScope_Global, 0, &startTime, UInt32(sizeof(startTime.dynamicType))), operation: "AudioUnitSetProperty[kAudioUnitProperty_ScheduleStartTimeStamp] failed")
